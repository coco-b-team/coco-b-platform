...
// quede centrada en vez de pegada a uno de los dos lados. Pensado para 4
// premios (el diseño real); si algún día hay más o menos, los que sobran
// simplemente no llevan línea divisoria en vez de romper el layout.
...
export function AwardsCarousel({ awards }: { awards: Award[] }) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16 sm:px-10">
      ...
      {/* Mobile: un premio a la vez, con flechas. Los premios están
      ...
      <div className="mt-8 hidden gap-10 sm:grid sm:grid-cols-2 sm:gap-0 lg:grid-cols-4">
        {awards.map((award, i) => (
          <div key={award.id} className={`lg:text-center ${DIVIDER_CLASSES[i] ?? ''}`}>
            <p className="text-sm tracking-widest text-accent">{award.year}</p>
            <p className="mt-1 text-lg font-semibold">{award.title}</p>
