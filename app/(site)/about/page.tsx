  ...
  return (
    <>
      <section className="mx-auto max-w-3xl px-6 py-16 sm:px-10">
        ...
        <div className="mt-10 grid grid-cols-1 gap-10 sm:grid-cols-3">
          {about.triad.map((item) => (
            <div key={item.title}>
              ...
              <p className="text-text-muted mt-3">{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div className="border-border text-text-muted mt-14 space-y-6 border-t pt-10">
          {storyParagraphs.map((paragraph) => (
            <p key={paragraph.slice(0, 24)}>{paragraph}</p>
          ))}
          <p className="text-text font-medium">{about.mission}</p>
        </div>

        <div className="border-border mt-12 flex flex-col items-center gap-6 border-t pt-10 text-center">
          <p className="text-accent text-sm tracking-[0.3em] uppercase">{about.tagline}</p>
      ...
      <section className="bg-background-alt py-16">
        <div className="mx-auto max-w-3xl px-6 sm:px-10">
          <p className="text-accent text-xs tracking-widest uppercase">{t('sustainability')}</p>
          <h2 className="font-body mt-1 text-2xl font-semibold">{about.sustainabilityHeading}</h2>
          <p className="text-text-muted mt-4">{about.sustainabilityIntro}</p>
          ...
          <div className="border-border text-text-muted mt-8 space-y-2 border-t pt-8">
